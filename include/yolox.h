...
public:
    // YoloX(){
    //     YoloX("yolox_nano.param","yolox_nano.bin",416,416,0.25,0.45);
    // }
    YoloX(float prob_thresh, std::string param_path = "yolox_nano.param",
          std::string bin_path = "yolox_nano.bin",
...
#pragma omp section
    ...
    void nms_sorted_bboxes(const std::vector<Object> &faceobjects, std::vector<int> &picked, float nms_thresh)
    {
        picked.clear();
    ...
    void generate_yolox_proposals(std::vector<GridAndStride> grid_strides, const ncnn::Mat &feat_blob, float prob_thresh, std::vector<Object> &objects)
    {
