...
void box_to_center_scale(cv::Rect_<float> rect, int width, int height,
...
void affine_tranform(float pt_x, float pt_y, cv::Mat& t, float* x, int p,
                     int num) {
...
cv::Mat get_affine_transform(std::vector<float>& center,
                             std::vector<float>& scale, float rot,
                             std::vector<int>& output_size, int inv) {
  ...
  std::vector<float> dst1{float(dst_w * 0.5) + dst_dir[0],
                          float(dst_h * 0.5) + dst_dir[1]};
  std::vector<float> src2 = get_3rd_point(center, src1);
  std::vector<float> dst2 = get_3rd_point(dst0, dst1);
  if (inv == 0) {
    float a[6][6] = {
        {center[0], center[1], 1, 0, 0, 0}, {0, 0, 0, center[0], center[1], 1},
        {src1[0], src1[1], 1, 0, 0, 0},     {0, 0, 0, src1[0], src1[1], 1},
        {src2[0], src2[1], 1, 0, 0, 0},     {0, 0, 0, src2[0], src2[1], 1}};
    ...
    float a[6][6] = {
        {dst0[0], dst0[1], 1, 0, 0, 0}, {0, 0, 0, dst0[0], dst0[1], 1},
        {dst1[0], dst1[1], 1, 0, 0, 0}, {0, 0, 0, dst1[0], dst1[1], 1},
        {dst2[0], dst2[1], 1, 0, 0, 0}, {0, 0, 0, dst2[0], dst2[1], 1}};
...
void transform_preds(float* coords, std::vector<float>& center,
                     std::vector<float>& scale, std::vector<int>& output_size,
                     int64_t t, float* target_coords) {
...
void get_final_preds(ncnn::Mat out, std::vector<float>& center,
                     std::vector<float> scale, float* preds) {
  ...
  for (int i = 0; i < 1; ++i) {
    for (int j = 0; j < out.c; ++j) {
      ...
      if (px > 1 && px < heatmap_width - 1 && py > 1 &&
          py < heatmap_height - 1) {
        float diff_x = out[index + py * heatmap_width + px + 1] -
                       out[index + py * heatmap_width + px - 1];
        float diff_y = out[index + (py + 1) * heatmap_width + px] -
                       out[index + (py - 1) * heatmap_width + px];
...
int SLPNet::load(const char* modeltype, int _target_size,
...
int SLPNet::detect(const cv::Mat& rgb, cv::Rect_<float> rect) {
  ...
  cv::warpAffine(rgb, input, tran, cv::Size(img_size[0], img_size[1]),
                 cv::INTER_LINEAR);

  _normalize(input);

  //    cv::imwrite("/storage/emulated/0/abcd/" + std::to_string(k1) + ".jpg",
  //    input);
  // k1++;
  // ncnn::Mat in = ncnn::Mat::from_pixels_resize(input.data,
  // ncnn::Mat::PIXEL_RGB, 192, 256, 192, 256);

  // ncnn::Mat in = ncnn::Mat::from_pixels(input.data, ncnn::Mat::PIXEL_RGB,
  // 192, 256);
  ...
  //    char text[256];
  //    sprintf(text, "%d %d %d %d %.0f %.0f %.2f %.2f %d %d", a1.dims, a1.h,
  //    a1.w, a1.c,a1.channel(0).row(10)[0],
  //            a[0*11*10+10*10], center[0], scale[0], img_w, img_h);
  //    cv::Scalar textcc = cv::Scalar(255, 255, 255);
  //    cv::putText(rgb, text, cv::Point(10, 100), cv::FONT_HERSHEY_SIMPLEX,
  //    0.5, textcc, 1);

  // so for 0-255 input image, rgb_mean should multiply 255 and norm should div
  // by std.
...

  {
    ...
    //        cv::Scalar textcc = cv::Scalar(255, 255, 255);
    //        cv::putText(rgb, text, cv::Point(100, 200),
    ...
    //        cv::putText(rgb, text, cv::Point(20, 300),
    //        cv::FONT_HERSHEY_SIMPLEX, 0.5, textcc, 1);
    ...
    for (int i = line_begin; i < line_end; i = i + 2) {
      cv::line(
          rgb,
          cv::Point2d(int(preds[pair_line[i]]), int(preds[pair_line[i] + 17])),
          cv::Point2d(int(preds[pair_line[i + 1]]),
                      int(preds[pair_line[i + 1] + 17])),
          cv::Scalar(0, 255, 0), 4);
    }
    ...
    for (int i = point_begin; i < 17; i = i + iter) {
      ...
      // cv::putText(rgb, std::to_string(i), cv::Point2d(x_coord, y_coord),
      // cv::FONT_HERSHEY_SIMPLEX, 0.5, textcc, 1);
      //            __android_log_print(ANDROID_LOG_DEBUG, "%s",
      //            std::to_string(i).c_str(),
      //            "%s",std::to_string(x_coord).c_str());
    }
    ...
    //        cv::Scalar textcc = cv::Scalar(255, 255, 255);
    //        cv::putText(rgb, text, cv::Point(100, 100),
    //        cv::FONT_HERSHEY_SIMPLEX, 0.5, textcc, 1);

    //        std::vector<int> strides = {8, 16, 32}; // might have stride=64
    //        std::vector<GridAndStride> grid_strides;
    //        generate_grids_and_stride(target_size, strides, grid_strides);
    //        generate_yolox_proposals(grid_strides, out, prob_threshold,
    //        proposals);
  }
...
int SLPNet::draw(cv::Mat& rgb) { return 0; }
